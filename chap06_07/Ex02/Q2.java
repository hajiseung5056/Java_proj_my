...
public class Q2 {
	...
	static void math (String...val) {
		...
		for(int i = 0; i < val.length ; i++) {
			...
			// sum = sum + Integer.parseInt(values[i]);
					// Integer.parseInt(String) : 숫자형String 타입을 정수로 변환
			sum += Integer.parseInt(val[i]);
		}
		
		avg = (double) sum / val.length; // 평균은 : double 타입이어야한다.
		System.out.println();
		System.out.println("입력한 값의수는 : " + val.length);
	...
	public static void main(String[] args) {
		...
		math(array1);    // 가변 길이 메소드 math를 호출하면서 매개변수로 array1을 넣어준다.
	}
